...
def criar_dados_exemplo(n_pontos=10, seed=42):
    ...
    valores = (
        2 + 0.05 * x + 0.1 * np.sin(0.1 * x) * np.cos(0.1 * y) + np.random.rand(n_pontos) * 0.5
    )
...
def interpolar_idw(pontos, valores, grid_x, grid_y, power=2.0, n_neighbors=None, max_distance=None):
    ...
    config = IDWConfig(power=power, n_neighbors=n_neighbors, max_distance=max_distance)
...
def interpolar_krigagem(pontos, valores, grid_x, grid_y, modelo="spherical"):
    ...
    config = KrigagemConfig(modelo_variograma=modelo, enable_statistics=True)
...
def visualizar_resultado(
    ...
    contour_lines = plt.contour(
        grid_x, grid_y, z, levels=10, colors="black", alpha=0.4, linewidths=0.5
    )
...
def main():
    ...
    parser.add_argument(
        "--resolucao", type=float, default=0.5, help="Resolução da grade (menor = mais detalhada)"
    )
    parser.add_argument(
        "--n_pontos", type=int, default=15, help="Número de pontos amostrais para dados de exemplo"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Semente para geração de dados aleatórios"
    )
    parser.add_argument(
        "--salvar", type=str, default=None, help="Caminho para salvar a figura (ex: resultado.png)"
    )

    # Argumentos específicos para IDW
    idw_group = parser.add_argument_group("Parâmetros IDW")
    idw_group.add_argument(
        "--power", type=float, default=2.0, help="Expoente da distância para IDW"
    )
    idw_group.add_argument("--vizinhos", type=int, default=None, help="Número de vizinhos para IDW")
    idw_group.add_argument("--dist_max", type=float, default=None, help="Distância máxima para IDW")

    # Argumentos específicos para Krigagem
    krig_group = parser.add_argument_group("Parâmetros Krigagem")
    krig_group.add_argument(
        "--modelo",
        type=str,
        default="spherical",
    ...
    pot_group.add_argument("--fluxo", action="store_true", help="Mostrar vetores de fluxo")
    ...
    try:
        ...
        # Criar grade regular
        grid_x, grid_y = criar_grade_regular(
            0, 50, 0, 50, int(50 / args.resolucao), int(50 / args.resolucao)
        )
        ...
        if args.metodo == "idw":
            ...
            z, ss = interpolar_krigagem(pontos, valores, grid_x, grid_y, modelo=args.modelo)
